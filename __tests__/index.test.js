...
describe("createMockStore", () => {
    it("should set the initial state to the passed-in value", () => {
        ...
        const mockStore = (0, src_1.createMockStore)(initialState, (set) => ({}));
        expect(mockStore.getState()).toEqual(initialState);
    });
    it("should update the state when setState is called", () => {
        const mockStore = (0, src_1.createMockStore)({ foo: "bar" }, (set) => ({}));
    ...
    it("should apply overrides to the initial state", () => {
        const mockStore = (0, src_1.createMockStore)({ incrementCount: 0 }, (set) => ({
            increment: () => set((state) => (Object.assign(Object.assign({}, state), { incrementCount: state.incrementCount + 1 }))),
        }), [{ incrementCount: 1 }]);
    ...
    it("should apply overrides to the custom actions", () => {
        const mockStore = (0, src_1.createMockStore)({ foo: "bar", count: 1 }, (set) => ({
            increment: () => set((state) => (Object.assign(Object.assign({}, state), { count: state.count + 1 }))),
        }), undefined, { increment: () => console.log("overridden!") });
        mockStore.increment();
